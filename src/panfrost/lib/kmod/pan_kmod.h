...
struct pan_kmod_ops {
   ...
   /* Import a dma-buf by its fd, for a kernel driver with no GEM handle namespace (kbase).
    * When set, it replaces drmPrimeFDToHandle() + bo_import(). The fd stays the caller's.
    * This method is optional.
    */
   struct pan_kmod_bo *(*bo_import_fd)(struct pan_kmod_dev *dev, int fd, uint64_t size);
