...
panfrost_init_shader_caps(struct panfrost_screen *screen)
{
   ...
   for (unsigned i = 0; i <= MESA_SHADER_COMPUTE; i++) {
      ...
      case MESA_SHADER_TESS_EVAL:
         /* Emulated as compute jobs on the Bifrost job manager (pan_gs.h). */
...
panfrost_init_compute_caps(struct panfrost_screen *screen)
{
   ...
   /* Bifrost and later run up to 256 invocations per workgroup in hardware;
    * larger workgroups, up to GL's minimum of 1024, are split in the compiler
    * (panfrost_shader_compile). A variable workgroup size is only known at
    * dispatch, so it stays within the hardware's.
    */
   if (dev->arch >= 6) {
    ...
    */
   caps->max_threads_per_block = dev->arch >= 6 ? 1024 : 128;
   ...
   caps->max_variable_threads_per_block =
      dev->arch >= 6 ? PAN_MAX_HW_WORKGROUP_SIZE : 1024; // TODO
}
...
panfrost_init_screen_caps(struct panfrost_screen *screen)
{
   ...
   /* kbase is not DRM, so DRM_CAP_PRIME above came back empty or failed. Its memory can import a
    * dma-buf (window buffers from gralloc) but not export one. */
   if (pan_kmod_dev_is_kbase(dev->kmod.dev))
      caps->dmabuf = DRM_PRIME_CAP_IMPORT;

   /* Our GL 3.x implementation is WIP */
   /* On kbase (Android phones) the GL 3.3 work is the point: turn it on there. */
   bool is_gl3 = (dev->debug & PAN_DBG_GL3) || pan_kmod_dev_is_kbase(dev->kmod.dev);
   ...
   /* Bifrost has the LOD query too (GRDESC, pan_nir_lower_tex.c). */
   caps->texture_query_lod = dev->arch >= 6;

   /* GL 4.5 pieces the hardware and compiler already have: fine/coarse derivatives, ballot-based
    * subgroup votes, textureSamples (a sysval on Bifrost), and texture barriers (a flush).
    * Votes need the warp mask (WMASK), which v6 does not run: on a Mali-G72 a fragment shader
    * with one draws nothing at all, whatever its subgroup size. */
   caps->fs_fine_derivative = dev->arch >= 6;
   caps->shader_group_vote = dev->arch >= 7;
   ...
   /* The GL version still follows the extensions: GLSL is clamped to it. */
   caps->glsl_feature_level =
   caps->glsl_feature_level_compatibility = is_gl3 ? 460 : 140;

   /* In software (panfrost_screen::nir_options). */
   caps->doubles = is_gl3 && dev->arch >= 6;
   ...
   /* Draws with a GPU-side count go through util_draw_indirect, which reads the count on the
    * CPU, like every indirect draw on v7 and older. */
   caps->multi_draw_indirect = true;
   caps->multi_draw_indirect_params = true;

   /* Cull distances are done per fragment, like clip distances (pan_shader.c). */
   ...
   /* Adjacency and patches only reach the geometry and tessellation
    * emulation, which assembles the input primitives itself. */
   if (dev->arch == 6 || dev->arch == 7) {
      ...
      /* Through the geometry shader emulation, the only stage that can pick
       * a viewport here. */
      caps->max_viewports = PAN_GS_MAX_VIEWPORTS;
   }
                       ...
                       struct renderonly *ro)
{
   ...
   for (unsigned i = 0; i <= MESA_SHADER_COMPUTE; i++) {
      screen->nir_options[i] = *pan_get_nir_shader_compiler_options(dev->arch, i, false);

      /* Mali has no fp64: every double operation becomes integer code from
       * Mesa's float64 library (ARB_gpu_shader_fp64). */
      if (dev->arch >= 6)
         screen->nir_options[i].lower_doubles_options |= nir_lower_fp64_full_software;

      screen->base.nir_options[i] = &screen->nir_options[i];
   }
