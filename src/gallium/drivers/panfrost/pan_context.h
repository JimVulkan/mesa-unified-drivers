...
struct panfrost_context {
   ...
   uint64_t tf_prims_overflowed;

   /* GPU addresses of the current draw's struct pan_gs_params, by the stage
    * the emulation runs (geometry, tess control, tess eval), and of its
    * struct poly_tess_params. */
   uint64_t emu_params[MESA_SHADER_STAGES];
   uint64_t tess_params;

   /* Every viewport and scissor (ARB_viewport_array); pipe_viewport and
    * scissor are the first ones. */
   struct pipe_viewport_state viewports[PAN_GS_MAX_VIEWPORTS];
   struct pipe_scissor_state scissors[PAN_GS_MAX_VIEWPORTS];

   /* Tessellation state: GL_PATCH_VERTICES, the default levels used when
    * there is no tess control shader, and the heap the tessellator
    * allocates from (created with the first tessellated draw). */
   uint8_t patch_vertices;
   float default_outer_level[4];
   float default_inner_level[2];
   struct panfrost_bo *tess_heap;
   uint64_t draw_calls;
   ...
   /* kbase: the last submission of this context, which its fences wait for. */
   uint64_t kbase_seq;
...
enum {
   ...
   PAN_SYSVAL_TEXTURE_SAMPLES = 21,
   /* Address of the draw's struct pan_gs_params (geometry shader emulation). */
   PAN_SYSVAL_GS_PARAMS = 22,
   /* Address of the draw's struct poly_tess_params (tessellation emulation). */
   PAN_SYSVAL_TESS_PARAMS = 23,
};
...
struct panfrost_fs_key {
   ...
   /* The last vertex stage writes gl_ViewportIndex: discard outside that
    * viewport (pan_gs.h). */
   bool viewport_clip;

   /* The last vertex stage is emulated: gl_Layer and gl_ViewportIndex come
    * in the emulation's slots (PAN_GS_VIEWPORT_SLOT). */
   bool emulated_producer;
...
struct panfrost_vs_key {
   ...
   /* A geometry shader follows: gl_Position stays in clip space, and the
    * varyings are written as they are (pan_gs.h). */
   bool keep_clip_space;
};

/* One draw through the geometry shader emulation (pan_gs.h): what the vertex,
 * compute and tiler jobs point at. */
struct panfrost_gs_draw {
   /* The vertex job: its varying buffers and ATTRIBUTE descriptors. */
   uint64_t vs_bufs, vs_producer;
   /* The tiler job: the emulation's buffers, the fragment shader's
    * descriptors into them, positions, point sizes and indices. */
   uint64_t gs_bufs, fs_consumer;
   uint64_t out_pos, out_psiz, out_index;
   uint32_t out_verts, index_count;
   /* Compute workgroups of PAN_GS_WORKGROUP_SIZE. */
   uint32_t wg;
   enum mesa_prim out_prim;
   /* A VIEWPORT descriptor covering every viewport, when the shader picks
    * them per primitive; 0 for the batch's own. */
   uint64_t viewport;
};

/* One tessellated draw (pan_gs.h): the tess eval stage's buffers are the
 * geometry shader emulation's, plus what the tess control job and the
 * tessellator need. */
struct panfrost_tess_draw {
   struct panfrost_gs_draw gs;
   /* The tess control grid: patches per instance, instances, and a
    * workgroup per patch of one invocation per output vertex. */
   uint32_t ppi, instances, tcs_local;
   uint32_t nr_patches;
   /* struct poly_tess_params, and where the prefix sum leaves the index
    * count. */
   uint64_t params, count;
   enum tess_primitive_mode prim;
};

struct panfrost_gs_key {
   /* As panfrost_vs_key::noperspective_varyings, for the geometry shader's
    * outputs. */
   uint32_t noperspective_varyings;
};
...
struct panfrost_uncompiled_shader {
   ...
   /* Geometry shaders, and tess eval shaders run as one: the emulation's
    * shape (pan_gs.h). */
   struct pan_gs_info gs;

   /* Tessellation shaders: the mode either stage may declare, and the tess
    * control shader's output record. */
   struct {
      enum tess_primitive_mode prim;
