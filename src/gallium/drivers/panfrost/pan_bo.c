...
panfrost_bo_free(struct panfrost_bo *bo)
{
   ...
   /* kbase frees the memory at once, not when the jobs using it are done. */
   if (pan_kmod_dev_is_kbase(bo->dev->kmod.dev))
...
panfrost_bo_wait(struct panfrost_bo *bo, int64_t timeout_ns, bool wait_readers)
{
   ...
   /* kbase has no per-BO wait: wait for the last submission that used the BO. That covers our
    * own GPU work only, which for a shared BO is all the other side's fences leave to us. */
   if (pan_kmod_dev_is_kbase(bo->dev->kmod.dev)) {
      if (!bo->gpu_access)
         return true;
      ...
      const int64_t abs = timeout_ns >= INT64_MAX - now ? INT64_MAX : now + timeout_ns;
      if (!pan_kmod_kbase_wait_seq_timeout(bo->dev->kmod.dev, bo->kbase_seq, abs))
         return false;
      if (bo->dev->stall_log) {
         const double ms = (os_time_get_nano() - now) / 1e6;
         if (ms > 2.0)
            mesa_logi("pan stall: %.1f ms waiting for BO \"%s\" (%zu KB)", ms,
                      bo->label ? bo->label : "?", (size_t)(panfrost_bo_size(bo) / 1024));
      }
      bo->gpu_access = 0;
      return true;
   }
