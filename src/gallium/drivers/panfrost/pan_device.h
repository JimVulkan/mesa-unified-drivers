...
struct panfrost_device {
   ...
   /* Sync objects: DRM syncobjs, or on kbase (no DRM node) the kbase backend's own. */
   struct util_sync_provider *sync;

   /* PAN_STALL_LOG (Android: debug.mesa.pan.stall.log): log CPU waits on the GPU and shader
    * compiles longer than a few milliseconds, to find frame hitches. */
   bool stall_log;
