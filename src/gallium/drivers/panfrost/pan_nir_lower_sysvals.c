...
sysval_for_intrinsic(unsigned arch, nir_intrinsic_instr *intr, unsigned *offset)
{
   ...
   case nir_intrinsic_load_geometry_param_buffer_poly:
      return PAN_SYSVAL_GS_PARAMS;

   case nir_intrinsic_load_tess_param_buffer_poly:
      return PAN_SYSVAL_TESS_PARAMS;
...
lower(nir_builder *b, nir_instr *instr, void *data)
{
   ...
   if (instr->type == nir_instr_type_intrinsic) {
      ...
      if (tex->op == nir_texop_texture_samples) {
         /* textureSamples (ARB_shader_texture_image_samples): Bifrost has no query for it. */
         sysval = PAN_SYSVAL(TEXTURE_SAMPLES, tex->texture_index);
