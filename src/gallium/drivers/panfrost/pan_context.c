...
panfrost_writes_point_size(struct panfrost_context *ctx)
{
   ...
   /* Behind a geometry shader the tiler draws what it emitted. */
   struct panfrost_uncompiled_shader *gs = ctx->uncompiled[MESA_SHADER_GEOMETRY];
   if (gs && pan_device(ctx->base.screen)->arch <= 7)
      return gs->gs.writes_psiz && ctx->active_prim == MESA_PRIM_POINTS;
                             ...
                             const struct pipe_viewport_state *viewports)
{
   ...
   for (unsigned i = 0; i < num_viewports; i++) {
      if (start_slot + i < ARRAY_SIZE(ctx->viewports))
         ctx->viewports[start_slot + i] = viewports[i];
   }

   ctx->pipe_viewport = ctx->viewports[0];
                            ...
                            const struct pipe_scissor_state *scissors)
{
   ...
   for (unsigned i = 0; i < num_scissors; i++) {
      if (start_slot + i < ARRAY_SIZE(ctx->scissors))
         ctx->scissors[start_slot + i] = scissors[i];
   }

   ctx->scissor = ctx->scissors[0];
...
panfrost_destroy(struct pipe_context *pipe)
{
   ...
   panfrost_bo_unreference(panfrost->tess_heap);
   ...
   dev->sync->destroy(dev->sync, panfrost->in_sync_obj);
   if (panfrost->in_sync_fd != -1) {
      close(panfrost->in_sync_fd);
      panfrost->in_sync_fd = -1;
   }

   dev->sync->destroy(dev->sync, panfrost->syncobj);
   ralloc_free(pipe);
}
...
panfrost_begin_query(struct pipe_context *pipe, struct pipe_query *q)
{
   ...
   case PIPE_QUERY_SO_OVERFLOW_ANY_PREDICATE:
      query->start = ctx->tf_prims_overflowed;
      break;
...
panfrost_end_query(struct pipe_context *pipe, struct pipe_query *q)
{
   ...
   case PIPE_QUERY_SO_OVERFLOW_PREDICATE:
   case PIPE_QUERY_SO_OVERFLOW_ANY_PREDICATE:
      query->end = ctx->tf_prims_overflowed;
      break;
   case PAN_QUERY_DRAW_CALLS:
                          ...
                          bool wait, union pipe_query_result *vresult)
{
   ...
   /* Transform feedback has one stream without geometry shaders. */
   case PIPE_QUERY_SO_OVERFLOW_PREDICATE:
   case PIPE_QUERY_SO_OVERFLOW_ANY_PREDICATE:
      vresult->b = query->end != query->start;
      break;
...
/* ARB_query_buffer_object, on the CPU: every result here is computed or read back on the CPU
 * after a wait, so the value (and an availability of 1) is written into the buffer like a
 * glBufferSubData, which stages the write when the GPU still uses the buffer. */
static void
                                   ...
                                   unsigned offset)
{
   ...
   if (index >= 0) {
      ...
      panfrost_get_query_result(pipe, q, true, &res);
   ...
   case PIPE_QUERY_TYPE_I32: {
      int32_t v = MIN2(value, INT32_MAX);
      pipe_buffer_write(pipe, resource, offset, sizeof(v), &v);
      break;
   }
   case PIPE_QUERY_TYPE_U32: {
      uint32_t v = MIN2(value, UINT32_MAX);
      pipe_buffer_write(pipe, resource, offset, sizeof(v), &v);
      break;
   }
   case PIPE_QUERY_TYPE_I64:
   case PIPE_QUERY_TYPE_U64:
      pipe_buffer_write(pipe, resource, offset, sizeof(value), &value);
      break;
   }
}

static void
panfrost_set_patch_vertices(struct pipe_context *pipe, uint8_t patch_vertices)
{
   pan_context(pipe)->patch_vertices = patch_vertices;
}

static void
panfrost_set_tess_state(struct pipe_context *pipe,
                        const float default_outer_level[4],
                        const float default_inner_level[2])
{
   struct panfrost_context *ctx = pan_context(pipe);

   memcpy(ctx->default_outer_level, default_outer_level,
          sizeof(ctx->default_outer_level));
   memcpy(ctx->default_inner_level, default_inner_level,
                           ...
                           uint64_t value)
{
   ...
   /* kbase runs this device's submissions in order, so only a foreign sync file needs waiting. */
   if (pan_kmod_dev_is_kbase(dev->kmod.dev)) {
      if (f->sync_fd >= 0)
         sync_accumulate("panfrost", &ctx->in_sync_fd, f->sync_fd);
      return;
   }

   ret = dev->sync->export_sync_file(dev->sync, f->syncobj, &fd);
   assert(!ret);
...
panfrost_create_context(struct pipe_screen *screen, void *priv, unsigned flags)
{
    ...
    */
   ret = dev->sync->create(dev->sync, DRM_SYNCOBJ_CREATE_SIGNALED,
                          &ctx->syncobj);
   if (ret) {
      ralloc_free(ctx);
      return NULL;
   ...
   gallium->set_patch_vertices = panfrost_set_patch_vertices;
   gallium->set_tess_state = panfrost_set_tess_state;
   ...
   ctx->in_sync_fd = -1;
   ret = dev->sync->create(dev->sync, 0, &ctx->in_sync_obj);
   assert(!ret);
