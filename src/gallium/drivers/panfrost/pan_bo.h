...
struct panfrost_bo {
   ...
   /* kbase: the last submission that accessed this BO (pan_kmod_kbase_submit_async). kbase has
    * no per-BO wait, so panfrost_bo_wait waits for that submission instead. */
   uint64_t kbase_seq;
