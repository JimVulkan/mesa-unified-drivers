   ...
   for (unsigned i = 0; i < ss->sysvals.sysval_count; ++i) {
      ...
      case PAN_SYSVAL_TEXTURE_SIZE:
         panfrost_upload_txs_sysval(batch, st, PAN_SYSVAL_ID(sysval),
                                    &uniforms[i]);
         break;
      case PAN_SYSVAL_TEXTURE_SAMPLES: {
         struct panfrost_sampler_view *view =
            batch->ctx->sampler_views[st][PAN_SYSVAL_ID(sysval)];
         uniforms[i].u[0] = view && view->base.texture
                               ? MAX2(view->base.texture->nr_samples, 1)
                               : 0;
         break;
      }
      ...
      case PAN_SYSVAL_GS_PARAMS:
         uniforms[i].du[0] = batch->ctx->emu_params[st];
         break;

      case PAN_SYSVAL_TESS_PARAMS:
         uniforms[i].du[0] = batch->ctx->tess_params;
         break;
...
/* Transform feedback writes vertex i of instance n at (n * count + i) * stride past the target's
 * offset, with no bounds check in the shader. GL stops writing at the first primitive that does
 * not fit in a bound buffer and reports it to the overflow queries, so the capture is cut here to
 * what fits: whole primitives without instancing, whole instances with it. */
static void
panfrost_xfb_clamp(struct panfrost_context *ctx, enum mesa_prim mode,
                   unsigned count, unsigned instances, unsigned *out_count,
                   unsigned *out_instances)
{
   *out_count = count;
   *out_instances = instances;

   struct panfrost_compiled_shader *vs = ctx->prog[MESA_SHADER_VERTEX];
   if (!vs || !ctx->streamout.num_targets)
      return;

   uint64_t fit = UINT64_MAX;
   for (unsigned i = 0; i < ctx->streamout.num_targets; i++) {
      struct pipe_stream_output_target *t = ctx->streamout.targets[i];
      uint64_t stride = vs->stream_output.stride[i] * 4;
      if (!t || !stride)
         continue;

      uint64_t used = pan_so_target(t)->offset * stride;
      fit = MIN2(fit, used >= t->buffer_size ? 0 : (t->buffer_size - used) / stride);
   }

   if ((uint64_t)count * instances <= fit)
      return;

   if (instances > 1) {
      *out_instances = count ? fit / count : 0;
   } else {
      unsigned vpp = mesa_vertices_per_prim(mode);
      *out_count = (mode == MESA_PRIM_POINTS || mode == MESA_PRIM_LINES ||
                           ...
                           const struct pipe_draw_start_count_bias *draw)
{
   ...
   uint64_t prims =
      (uint64_t)u_prims_for_vertices(info->mode, draw->count) * info->instance_count;
   ...
   panfrost_xfb_clamp(ctx, info->mode, draw->count, info->instance_count, &count,
                      &instances);
   uint64_t written = (uint64_t)u_prims_for_vertices(info->mode, count) * instances;

   ctx->tf_prims_generated += written;
   ctx->tf_prims_overflowed += prims - written;
   ctx->dirty |= PAN_DIRTY_SO;
}

static void
panfrost_update_streamout_offsets(struct panfrost_context *ctx, unsigned count)
{
   for (unsigned i = 0; i < ctx->streamout.num_targets; ++i) {
                    ...
                    const struct pipe_draw_info *info, unsigned count)
{
   ...
   unsigned instances;
   panfrost_xfb_clamp(ctx, info->mode, count, info->instance_count, &count,
                      &instances);
   if (!count || !instances)
      return;

   struct pipe_draw_info clamped = *info;
   clamped.instance_count = instances;
   info = &clamped;
...
/*
 * Geometry shaders, emulated as a compute job between the vertex and tiler jobs (pan_gs.h).
 */
static enum pan_gs_topology
panfrost_gs_topology(enum mesa_prim mode)
{
   switch (mode) {
   case MESA_PRIM_LINE_STRIP:
   case MESA_PRIM_TRIANGLE_STRIP:
   case MESA_PRIM_LINE_STRIP_ADJACENCY:
      return PAN_GS_TOPO_STRIP;
   case MESA_PRIM_TRIANGLE_FAN:
      return PAN_GS_TOPO_FAN;
   case MESA_PRIM_LINE_LOOP:
      return PAN_GS_TOPO_LOOP;
   case MESA_PRIM_TRIANGLE_STRIP_ADJACENCY:
      return PAN_GS_TOPO_TRI_STRIP_ADJ;
   default:
      return PAN_GS_TOPO_LIST;
   }
}

static unsigned
panfrost_gs_prims_per_instance(enum mesa_prim mode, unsigned n, unsigned vpp)
{
   switch (panfrost_gs_topology(mode)) {
   case PAN_GS_TOPO_STRIP:
      return n >= vpp ? n - (vpp - 1) : 0;
   case PAN_GS_TOPO_FAN:
      return n >= 3 ? n - 2 : 0;
   case PAN_GS_TOPO_LOOP:
      return n >= 2 ? n : 0;
   case PAN_GS_TOPO_TRI_STRIP_ADJ:
      return n >= 6 ? (n - 4) / 2 : 0;
   default:
      return n / vpp;
   }
...
static mali_pixel_format
panfrost_gs_rgba32f(void)
{
   mali_pixel_format format = MALI_RGBA32F << 12;
#if PAN_ARCH <= 6
   format |= pan_get_default_swizzle(4);
#endif
   return format;
}

/* The vertex job's side: buffers for everything the vertex shader writes, positions unsnapped in
 * clip space, and the slot table the emulation reads them with. */
static bool
panfrost_gs_emit_vs_varyings(struct panfrost_batch *batch, struct pan_gs_params *params,
                             struct panfrost_gs_draw *out)
{
   struct panfrost_context *ctx = batch->ctx;
   struct panfrost_device *dev = pan_device(ctx->base.screen);
   struct panfrost_compiled_shader *vs = ctx->prog[MESA_SHADER_VERTEX];
   const struct pan_varying_layout *layout = &vs->info.varyings.formats;
   pan_varying_layout_require_layout(layout);

   const unsigned count = ctx->padded_count * ctx->instance_count;
   unsigned present = BITFIELD_BIT(PAN_VARY_GENERAL) | BITFIELD_BIT(PAN_VARY_POSITION);
   if (vs->info.vs.writes_point_size)
      present |= BITFIELD_BIT(PAN_VARY_PSIZ);

   struct pan_ptr bufs =
      pan_pool_alloc_desc_array(&batch->pool.base, util_bitcount(present) + 1, ATTRIBUTE_BUFFER);
   struct pan_ptr descs =
      pan_pool_alloc_desc_array(&batch->pool.base, MAX2(layout->count, 1), ATTRIBUTE);
   struct pan_ptr table = pan_pool_alloc_aligned(&batch->pool.base, 64 * 4, 4);
   if (!bufs.cpu || !descs.cpu || !table.cpu)
      return false;

   struct mali_attribute_buffer_packed *b = bufs.cpu;
   memset(b + util_bitcount(present), 0, sizeof(*b));

   const unsigned stride = layout->generic_size_B;
   if (stride) {
      params->in_general = panfrost_bi_emit_varyings_buffer(
         batch, &b[pan_varying_index(present, PAN_VARY_GENERAL)], stride, count);
   } else {
      memset(&b[pan_varying_index(present, PAN_VARY_GENERAL)], 0, sizeof(*b));
   }
   params->in_general_stride = stride;
   params->in_pos = panfrost_bi_emit_varyings_buffer(
      batch, &b[pan_varying_index(present, PAN_VARY_POSITION)], 16, count);
   if (present & BITFIELD_BIT(PAN_VARY_PSIZ)) {
      params->in_psiz = panfrost_bi_emit_varyings_buffer(
         batch, &b[pan_varying_index(present, PAN_VARY_PSIZ)], 2, count);
   }

   uint32_t *offsets = table.cpu;
   for (unsigned i = 0; i < 64; i++)
      offsets[i] = PAN_GS_SLOT_UNUSED;

   /* Every slot the vertex shader writes is kept: the geometry shader may read any of them. */
   struct mali_attribute_packed *d = descs.cpu;
   for (unsigned i = 0; i < layout->count; i++) {
      const struct pan_varying_slot *slot = pan_varying_layout_slot_at(layout, i);
      if (!slot) {
         pan_emit_vary(dev, &d[i], 0, (MALI_CONSTANT << 12), 0);
         continue;
      }

      if (slot->location == VARYING_SLOT_POS) {
         pan_emit_vary(dev, &d[i], pan_varying_index(present, PAN_VARY_POSITION),
                       panfrost_gs_rgba32f(), 0);
      } else if (slot->location == VARYING_SLOT_PSIZ) {
         pan_emit_vary_special(dev, &d[i], present, PAN_VARY_PSIZ);
      } else if (slot->section == PAN_VARYING_SECTION_GENERIC) {
         enum pipe_format format = pan_varying_format(slot->alu_type, slot->ncomps);
         pan_emit_vary(dev, &d[i], 0, GENX(pan_format_from_pipe_format)(format)->hw,
                       slot->offset);
         if (slot->location < 64) {
            offsets[slot->location] =
               slot->offset |
               (nir_alu_type_get_type_size(slot->alu_type) == 16 ? PAN_GS_IN_16BIT : 0);
         }
      } else {
         pan_emit_vary(dev, &d[i], 0, (MALI_CONSTANT << 12), 0);
      }
   ...
   params->in_offsets = table.gpu;
   out->vs_bufs = bufs.gpu;
   out->vs_producer = layout->count ? descs.gpu : 0;
   return true;
}

/* The tiler job's side: the emulation's buffers, sized for every invocation filling its whole
 * slot, one dead vertex past the end, and the fragment shader's descriptors into them. */
static bool
panfrost_gs_emit_out_varyings(struct panfrost_batch *batch, const struct pan_gs_info *gsi,
                              struct pan_gs_params *params, struct panfrost_gs_draw *out)
{
   struct panfrost_context *ctx = batch->ctx;
   struct panfrost_device *dev = pan_device(ctx->base.screen);
   struct panfrost_compiled_shader *fs = ctx->prog[MESA_SHADER_FRAGMENT];
   ...
   /* Not the varying pool: the vertex shader reaches that through the varying unit, and plain
    * compute stores into it do not land (panvk measured it). */
   struct pan_ptr pos = pan_pool_alloc_aligned(&batch->pool.base, out->out_verts * 16, 64);
   struct pan_ptr gen =
      pan_pool_alloc_aligned(&batch->pool.base, out->out_verts * gsi->out_stride, 64);
   struct pan_ptr psiz = {0};
   if (gsi->writes_psiz)
      psiz = pan_pool_alloc_aligned(&batch->pool.base, ALIGN_POT(out->out_verts * 2, 4), 64);
   struct pan_ptr idx = pan_pool_alloc_aligned(&batch->pool.base, out->index_count * 4, 64);
   if (!bufs.cpu || !pos.cpu || !gen.cpu || !idx.cpu || (gsi->writes_psiz && !psiz.cpu))
      return false;

   struct mali_attribute_buffer_packed *b = bufs.cpu;
   memset(b + util_bitcount(present), 0, sizeof(*b));
   pan_pack(&b[pan_varying_index(present, PAN_VARY_GENERAL)], ATTRIBUTE_BUFFER, cfg) {
      cfg.stride = gsi->out_stride;
      cfg.size = out->out_verts * gsi->out_stride;
      cfg.pointer = gen.gpu;
   }
   pan_pack(&b[pan_varying_index(present, PAN_VARY_POSITION)], ATTRIBUTE_BUFFER, cfg) {
      cfg.stride = 16;
      cfg.size = out->out_verts * 16;
      cfg.pointer = pos.gpu;
   }
   if (gsi->writes_psiz) {
      pan_pack(&b[pan_varying_index(present, PAN_VARY_PSIZ)], ATTRIBUTE_BUFFER, cfg) {
         cfg.stride = 2;
         cfg.size = out->out_verts * 2;
         cfg.pointer = psiz.gpu;
      }
   }

   /* The dead vertex, far outside any viewport. */
   const unsigned dead = out->out_verts - 1;
   float *dead_pos = (float *)pos.cpu + dead * 4;
   dead_pos[0] = dead_pos[1] = -1.0e6f;
   dead_pos[2] = 0.0f;
   dead_pos[3] = 1.0f;

   uint64_t consumer = 0;
   if (fs) {
      const struct pan_varying_layout *fs_layout = &fs->info.varyings.formats;
      pan_varying_layout_require_format(fs_layout);

      if (fs_layout->count) {
         struct pan_ptr descs =
            pan_pool_alloc_desc_array(&batch->pool.base, fs_layout->count, ATTRIBUTE);
         if (!descs.cpu)
            return false;

         struct mali_attribute_packed *d = descs.cpu;
         for (unsigned i = 0; i < fs_layout->count; i++) {
            const struct pan_varying_slot *slot = pan_varying_layout_slot_at(fs_layout, i);
            if (!slot) {
               pan_emit_vary(dev, &d[i], 0, (MALI_CONSTANT << 12), 0);
               continue;
            }

            const uint32_t offset = pan_gs_out_offset(gsi, slot->location);
            enum pipe_format format = PIPE_FORMAT_NONE;
            if (offset != PAN_GS_SLOT_UNUSED) {
               nir_alu_type base = nir_alu_type_get_base_type(slot->alu_type);
               format = pan_varying_format(base | 32, slot->ncomps);
            }

            panfrost_bi_emit_varying(dev, &d[i], slot->location, format, present, 0,
                                     offset == PAN_GS_SLOT_UNUSED ? 0 : offset,
                                     PAN_VARY_FRAGCOORD);
         }
         consumer = descs.gpu;
      }
   ...
   params->out_pos = pos.gpu;
   params->out_psiz = psiz.gpu;
   params->out_general = gen.gpu;
   params->out_index = idx.gpu;
   params->dead_vertex = dead;

   out->gs_bufs = bufs.gpu;
   out->fs_consumer = consumer;
   out->out_pos = pos.gpu;
   out->out_psiz = psiz.gpu;
   out->out_index = idx.gpu;
   return true;
}

/* A geometry shader picking viewports: each one's transform for the emulation, each one's
 * rectangle for the fragment shader (panfrost_lower_viewport_clip_fs), and a tiler viewport
 * covering them all. */
static bool
panfrost_gs_emit_viewports(struct panfrost_batch *batch, struct pan_gs_params *params,
                           struct panfrost_gs_draw *out)
{
   struct panfrost_context *ctx = batch->ctx;
   const struct pipe_rasterizer_state *rast = &ctx->rasterizer->base;

   struct pan_ptr xf = pan_pool_alloc_aligned(&batch->pool.base, PAN_GS_MAX_VIEWPORTS * 24, 16);
   struct pan_ptr rects =
      pan_pool_alloc_aligned(&batch->pool.base, PAN_GS_MAX_VIEWPORTS * 16, 16);
   struct pan_ptr T = pan_pool_alloc_desc(&batch->pool.base, VIEWPORT);
   if (!xf.cpu || !rects.cpu || !T.cpu)
      return false;

   float *x = xf.cpu, *r = rects.cpu;
   unsigned uminx = UINT_MAX, uminy = UINT_MAX, umaxx = 0, umaxy = 0;
   float uminz = 1.0f, umaxz = 0.0f;

   for (unsigned i = 0; i < PAN_GS_MAX_VIEWPORTS; i++) {
      const struct pipe_viewport_state *vp = &ctx->viewports[i];
      const struct pipe_scissor_state *ss = &ctx->scissors[i];

      memcpy(x + i * 6, vp->scale, 12);
      memcpy(x + i * 6 + 3, vp->translate, 12);

      unsigned minx = MIN2(batch->key.width, MAX2((int)(vp->translate[0] - fabsf(vp->scale[0])), 0));
      unsigned maxx = MIN2(batch->key.width, MAX2((int)(vp->translate[0] + fabsf(vp->scale[0])), 0));
      unsigned miny = MIN2(batch->key.height, MAX2((int)(vp->translate[1] - fabsf(vp->scale[1])), 0));
      unsigned maxy = MIN2(batch->key.height, MAX2((int)(vp->translate[1] + fabsf(vp->scale[1])), 0));
      if (rast->scissor) {
         minx = MAX2(ss->minx, minx);
         miny = MAX2(ss->miny, miny);
         maxx = MIN2(ss->maxx, maxx);
      ...
      if (minx < maxx && miny < maxy) {
         uminx = MIN2(uminx, minx);
         uminy = MIN2(uminy, miny);
         umaxx = MAX2(umaxx, maxx);
         umaxy = MAX2(umaxy, maxy);
      }

      float minz, maxz;
      util_viewport_zmin_zmax(vp, rast->clip_halfz, &minz, &maxz);
      uminz = MIN2(uminz, SATURATE(minz));
      umaxz = MAX2(umaxz, SATURATE(maxz));
   }

   if (uminx >= umaxx || uminy >= umaxy)
      uminx = uminy = umaxx = umaxy = 1;

   panfrost_batch_union_scissor(batch, uminx, uminy, umaxx, umaxy);

   pan_cast_and_pack(T.cpu, VIEWPORT, cfg) {
      cfg.scissor_minimum_x = uminx;
      cfg.scissor_minimum_y = uminy;
      cfg.scissor_maximum_x = umaxx - 1;
      cfg.scissor_maximum_y = umaxy - 1;
      cfg.minimum_z = uminz;
      cfg.maximum_z = MAX2(umaxz, uminz);
   }

   params->viewports = xf.gpu;
   ctx->emu_params[MESA_SHADER_FRAGMENT] = rects.gpu;
   out->viewport = T.gpu;
   return true;
}

/* Everything the three jobs need, and the draw's pan_gs_params for the GS_PARAMS sysval. Returns
 * false when the draw has no complete input primitive, or an allocation failed. */
static bool
panfrost_gs_prepare(struct panfrost_batch *batch, const struct pipe_draw_info *info,
                    const struct pipe_draw_start_count_bias *draw, struct panfrost_gs_draw *out)
{
   struct panfrost_context *ctx = batch->ctx;
   const struct pan_gs_info *gsi = &ctx->uncompiled[MESA_SHADER_GEOMETRY]->gs;

   const unsigned vpp = MAX2(gsi->input_verts, 1);
   const unsigned ppi = panfrost_gs_prims_per_instance(info->mode, draw->count, vpp);
   const uint64_t in_prims = (uint64_t)ppi * info->instance_count;
   const uint64_t invocations = in_prims * gsi->invocations;
   if (!invocations || !gsi->max_verts || !gsi->max_prims)
      return false;

   const uint64_t out_verts = invocations * gsi->max_verts + 1;
   const uint64_t index_count = invocations * gsi->max_prims * gsi->output_verts;
   /* The worst case has to fit a 32-bit index and a sane allocation. */
   if (out_verts * (16 + gsi->out_stride) > (512ull << 20) || index_count > UINT32_MAX) {
      mesa_loge("geometry shader draw too large to emulate: %" PRIu64 " invocations",
                invocations);
      return false;
   }
   ...
   out->wg = DIV_ROUND_UP(invocations, PAN_GS_WORKGROUP_SIZE);
   out->out_prim = gsi->output_verts == 3   ? MESA_PRIM_TRIANGLES
                   : gsi->output_verts == 2 ? MESA_PRIM_LINES
                                            : MESA_PRIM_POINTS;

   struct pan_ptr p = pan_pool_alloc_aligned(&batch->pool.base, sizeof(struct pan_gs_params), 16);
   if (!p.cpu)
      return false;
   struct pan_gs_params *params = p.cpu;
   memset(params, 0, sizeof(*params));

   if (!panfrost_gs_emit_vs_varyings(batch, params, out) ||
       !panfrost_gs_emit_out_varyings(batch, gsi, params, out))
      return false;

   if (gsi->writes_viewport && !panfrost_gs_emit_viewports(batch, params, out))
      return false;

   params->index_buf = info->index_size ? batch->indices : 0;
   params->index_size = info->index_size;
   params->topology = panfrost_gs_topology(info->mode);
   params->prims_per_instance = ppi;
   params->instance_stride = info->instance_count > 1 ? ctx->padded_count : 0;
   params->num_input_prims = in_prims;
   params->index_bias = info->index_size ? ctx->offset_start - draw->index_bias : 0;
   params->vertex_count = draw->count;
   params->flatshade_first = ctx->rasterizer->base.flatshade_first;

   ctx->emu_params[MESA_SHADER_GEOMETRY] = p.gpu;
   return true;
}

/* Tessellation (pan_gs.h). 64 is the largest tessellation level, where a quad domain is 64x64
 * quads of two triangles; the tess eval job is sized for that, capped. */
#define PAN_TESS_HEAP_SIZE   (64u << 20)
#define PAN_TESS_MAX_INDICES (1u << 18)

static bool
panfrost_tess_prepare(struct panfrost_batch *batch, const struct pipe_draw_info *info,
                      const struct pipe_draw_start_count_bias *draw,
                      struct panfrost_tess_draw *out)
{
   struct panfrost_context *ctx = batch->ctx;
   struct panfrost_device *dev = pan_device(ctx->base.screen);
   struct panfrost_uncompiled_shader *tcs = ctx->uncompiled[MESA_SHADER_TESS_CTRL];
   struct panfrost_uncompiled_shader *tes = ctx->uncompiled[MESA_SHADER_TESS_EVAL];

   /* TODO: a pass-through tess control shader when there is none. */
   if (!tcs)
      return false;

   const unsigned pcp = MAX2(ctx->patch_vertices, 1);
   const unsigned ppi = draw->count / pcp;
   const unsigned instances = info->instance_count;
   const uint64_t nr_patches = (uint64_t)ppi * instances;
   if (!nr_patches || nr_patches > 0xffffff)
      return false;

   /* The mode: either stage may declare it, the tess eval shader wins. */
   const enum tess_primitive_mode prim = tes->tess.prim ? tes->tess.prim : tcs->tess.prim;
   const enum gl_tess_spacing spacing = tes->tess.spacing ? tes->tess.spacing : tcs->tess.spacing;
   const bool ccw = tes->tess.ccw || tcs->tess.ccw;
   const bool points = tes->tess.point_mode || tcs->tess.point_mode;

   memset(out, 0, sizeof(*out));
   const uint64_t cap = MIN2(nr_patches * 64 * 64 * 6, PAN_TESS_MAX_INDICES);
   out->gs.out_verts = cap + 1;
   out->gs.index_count = cap;
   out->gs.wg = DIV_ROUND_UP(cap, PAN_GS_WORKGROUP_SIZE);
   out->gs.out_prim = points                          ? MESA_PRIM_POINTS
                      : prim == TESS_PRIMITIVE_ISOLINES ? MESA_PRIM_LINES
                                                        : MESA_PRIM_TRIANGLES;
   out->prim = prim;
   out->ppi = ppi;
   out->instances = instances;
   out->nr_patches = nr_patches;
   out->tcs_local = MAX2(tcs->tess.vertices_out, 1);

   /* The heap the tessellator allocates domain points and indices from. Every batch starts at
    * its bottom: a batch's tessellation is consumed by its own tess eval job. */
   if (!ctx->tess_heap) {
      ctx->tess_heap = panfrost_bo_create(dev, PAN_TESS_HEAP_SIZE, 0, "Tessellation heap");
      if (!ctx->tess_heap)
         return false;
   }
   panfrost_batch_add_bo(batch, ctx->tess_heap, MESA_SHADER_VERTEX);

   if (!batch->tess_heap) {
      struct pan_ptr h = pan_pool_alloc_aligned(&batch->pool.base, sizeof(struct poly_heap), 16);
      if (!h.cpu)
         return false;
      *(struct poly_heap *)h.cpu = (struct poly_heap){
         .base = ctx->tess_heap->ptr.gpu,
         .bottom = 0,
         .size = PAN_TESS_HEAP_SIZE,
      };
      batch->tess_heap = h.gpu;
   }

   /* The tess control output records, the per-patch arrays, and the parameters. */
   const unsigned stride = tcs->tess.output_stride;
   uint32_t size = nr_patches * stride;
   ...
   size = draws_off + 32;
   const uint32_t count_off = size;
   size += 16;
   const uint32_t params_off = ALIGN_POT(size, 16);
   size = params_off + sizeof(struct poly_tess_params);

   struct pan_ptr blob = pan_pool_alloc_aligned(&batch->pool.base, size, 64);
   if (!blob.cpu)
      return false;
   ...
   *p = (struct poly_tess_params){
      ...
      .patch_coord_buffer = ctx->tess_heap->ptr.gpu,
   ...
   /* GL's domain origin is the lower left. */
   if (!points && prim != TESS_PRIMITIVE_ISOLINES)
      p->ccw = !ccw;
   memcpy(p->tess_level_outer_default, ctx->default_outer_level,
          sizeof(ctx->default_outer_level));
   memcpy(p->tess_level_inner_default, ctx->default_inner_level,
          sizeof(ctx->default_inner_level));

   out->params = blob.gpu + params_off;
   out->count = blob.gpu + count_off;
   ctx->tess_params = out->params;

   /* The tess control stage reads the vertex shader's records as the geometry shader emulation
    * does, a LIST of patches. */
   struct pan_ptr tp = pan_pool_alloc_aligned(&batch->pool.base, sizeof(struct pan_gs_params), 16);
   struct pan_ptr ep = pan_pool_alloc_aligned(&batch->pool.base, sizeof(struct pan_gs_params), 16);
   if (!tp.cpu || !ep.cpu)
      return false;
   struct pan_gs_params *tcs_params = tp.cpu, *tes_params = ep.cpu;
   memset(tcs_params, 0, sizeof(*tcs_params));
   memset(tes_params, 0, sizeof(*tes_params));

   if (!panfrost_gs_emit_vs_varyings(batch, tcs_params, &out->gs) ||
       !panfrost_gs_emit_out_varyings(batch, &tes->gs, tes_params, &out->gs))
      return false;

   tcs_params->index_buf = info->index_size ? batch->indices : 0;
   tcs_params->index_size = info->index_size;
   tcs_params->topology = PAN_GS_TOPO_LIST;
   tcs_params->prims_per_instance = ppi;
   tcs_params->instance_stride = instances > 1 ? ctx->padded_count : 0;
   tcs_params->index_bias = info->index_size ? ctx->offset_start - draw->index_bias : 0;
   tcs_params->patch_size = pcp;

   /* The tess eval stage: one invocation per tessellator index, up to the count the prefix sum
    * leaves. */
   tes_params->num_input_prims = cap;
   tes_params->num_prims_addr = out->count;
   tes_params->topology = PAN_GS_TOPO_LIST;
   tes_params->flatshade_first = ctx->rasterizer->base.flatshade_first;

   ctx->emu_params[MESA_SHADER_TESS_CTRL] = tp.gpu;
   ctx->emu_params[MESA_SHADER_TESS_EVAL] = ep.gpu;
   return true;
}
#endif
                            ...
                            const struct pipe_draw_start_count_bias *draw)
{
   ...
   struct panfrost_uncompiled_shader *tes = ctx->uncompiled[MESA_SHADER_TESS_EVAL];
   struct panfrost_gs_draw gs_draw;
   struct panfrost_tess_draw tess_draw;
   struct pipe_grid_info gs_grid = {.block = {PAN_GS_WORKGROUP_SIZE, 1, 1}, .work_dim = 1};
   struct pipe_grid_info tcs_grid = {.work_dim = 2};

   /* TODO: a geometry shader after tessellation. */
   if (tes && gs) {
      perf_debug(ctx, "tessellation followed by a geometry shader is not emulated yet");
      return;
   }

   /* The tiler draws what the geometry shader emits. */
   enum mesa_prim raster_prim = info->mode;
   if (gs) {
      raster_prim = gs->gs.output_verts == 3   ? MESA_PRIM_TRIANGLES
                    : gs->gs.output_verts == 2 ? MESA_PRIM_LINES
                                               : MESA_PRIM_POINTS;
   } else if (tes) {
      raster_prim = tes->tess.point_mode ? MESA_PRIM_POINTS
                    : tes->tess.prim == TESS_PRIMITIVE_ISOLINES ? MESA_PRIM_LINES
                                                                : MESA_PRIM_TRIANGLES;
   }
   panfrost_update_active_prim(ctx, raster_prim);
#else
...
#if PAN_ARCH == 6 || PAN_ARCH == 7
   if (gs) {
      /* The vertex shader's descriptor the generic check also wants is
       * emitted below. */
      if (ctx->rasterizer->base.rasterizer_discard ||
          batch->scissor_culls_everything ||
          !panfrost_gs_prepare(batch, info, draw, &gs_draw))
         return;

      /* The emulation's compute sysvals, and its parameters every draw. */
      gs_grid.grid[0] = gs_draw.wg;
      gs_grid.grid[1] = gs_grid.grid[2] = 1;
      ctx->compute_grid = &gs_grid;
      ctx->dirty_shader[MESA_SHADER_GEOMETRY] |= PAN_DIRTY_STAGE_CONST;
   } else if (tes) {
      if (ctx->rasterizer->base.rasterizer_discard ||
          batch->scissor_culls_everything ||
          !panfrost_tess_prepare(batch, info, draw, &tess_draw))
         return;

      gs_grid.grid[0] = tess_draw.gs.wg;
      gs_grid.grid[1] = gs_grid.grid[2] = 1;
      tcs_grid.grid[0] = tess_draw.ppi;
      tcs_grid.grid[1] = tess_draw.instances;
      tcs_grid.grid[2] = 1;
      tcs_grid.block[0] = tess_draw.tcs_local;
      tcs_grid.block[1] = tcs_grid.block[2] = 1;
      ctx->dirty_shader[MESA_SHADER_TESS_CTRL] |= PAN_DIRTY_STAGE_CONST;
      ctx->dirty_shader[MESA_SHADER_TESS_EVAL] |= PAN_DIRTY_STAGE_CONST;
   }
#endif
...
#if PAN_ARCH == 6 || PAN_ARCH == 7
   if (gs) {
      panfrost_update_shader_state(batch, MESA_SHADER_GEOMETRY);
      ctx->compute_grid = NULL;
   } else if (tes) {
      ctx->compute_grid = &tcs_grid;
      panfrost_update_shader_state(batch, MESA_SHADER_TESS_CTRL);
      ctx->compute_grid = &gs_grid;
      panfrost_update_shader_state(batch, MESA_SHADER_TESS_EVAL);
      ctx->compute_grid = NULL;
   }
#endif
...
#if PAN_ARCH == 6 || PAN_ARCH == 7
   if (gs) {
      if (vertex_count > 0 && batch->rsd[MESA_SHADER_VERTEX])
         GENX(jm_launch_draw_gs)(batch, info, vertex_count, &gs_draw);
      batch->draw_count++;
      return;
   } else if (tes) {
      if (vertex_count > 0 && batch->rsd[MESA_SHADER_VERTEX])
         GENX(jm_launch_draw_tess)(batch, info, vertex_count, &tess_draw);
      batch->draw_count++;
      return;
   }
#endif
   ...
   /* Increment transform feedback offsets by what was written */
   if (ctx->streamout.num_targets) {
      unsigned xfb_count, xfb_instances;
      panfrost_xfb_clamp(ctx, info->mode, draw->count, info->instance_count,
                         &xfb_count, &xfb_instances);
      panfrost_update_streamout_offsets(
         ctx, u_stream_outputs_for_vertices(info->mode, xfb_count) * xfb_instances);
   }
                       ...
                       const struct pipe_draw_indirect_info *indirect)
{
   ...
   if (!PAN_GPU_SUPPORTS_DRAW_INDIRECT || indirect->indirect_draw_count ||
       ctx->uncompiled[MESA_SHADER_GEOMETRY] ||
   ...
   panfrost_update_streamout_offsets(
      ctx, u_stream_outputs_for_vertices(ctx->active_prim, ctx->vertex_count));
                  ...
                  unsigned num_draws)
{
   ...
   /* Adjacency is only meaningful to a geometry shader, and only the
    * emulation (pan_gs.h) can assemble it. */
   if (info->mode >= MESA_PRIM_LINES_ADJACENCY &&
       info->mode <= MESA_PRIM_TRIANGLE_STRIP_ADJACENCY &&
       !ctx->uncompiled[MESA_SHADER_GEOMETRY])
      return;

   /* The emulation assembles primitives itself and does not see restarts:
    * split the draw at them on the CPU. */
   if (ctx->uncompiled[MESA_SHADER_GEOMETRY] && info->primitive_restart &&
       info->index_size && !(indirect && indirect->buffer) &&
       info->mode != MESA_PRIM_POINTS && info->mode != MESA_PRIM_LINES &&
      ...
      for (unsigned i = 0; i < num_draws; i++) {
         util_draw_vbo_without_prim_restart(
            pipe, info, drawid_offset + (info->increment_draw_id ? i : 0), NULL,
            &draws[i]);
      }
      return;
   }
                              ...
                              const struct pipe_grid_info *info)
{
   ...
   /* A workgroup larger than the hardware's was compiled to run as a smaller
    * 1D one (panfrost_shader_compile); dispatch it at that size.
    */
   struct pipe_grid_info real_grid;
   struct panfrost_compiled_shader *cs = ctx->prog[MESA_SHADER_COMPUTE];
   if (cs && cs->info.cs.real_workgroup_size) {
      real_grid = *info;
      real_grid.block[0] = cs->info.cs.real_workgroup_size;
      real_grid.block[1] = 1;
      real_grid.block[2] = 1;
      info = &real_grid;
   }
